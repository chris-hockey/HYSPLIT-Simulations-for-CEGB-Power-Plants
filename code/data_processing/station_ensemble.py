"""
Sub-annual HYSPLIT treatment assignment at monitoring-station level (ensemble).

Daily analogue of 3_monitoring_station_hysplit_assignment.py. The annual script
samples one annual kernel per plant-year and forms

    E_st = Σ_j f_jt · K_j(s,t).

This samples each plant-member DAILY cdump once and forms, for financial year t,
ensemble member m and fuel g:

    E^{(m,g)}_{sd} = Σ_{j∈g} f_jt · c_j(s,d)

the same fuel-input-weighted sum of station-cell concentrations, but per day.
Monthly and annual panels are means of the daily panel: fuel input is constant
within the year, so average-then-scale = scale-then-average, and the annual mean
here equals the annual-kernel measure exactly (the annual kernel is itself the
within-year mean of the daily concentrations).

Each cdump is opened exactly once. The plant and member come from the file name
(cdump_<plant>_<year>_k<n>.nc); the fuel and fuel input come from a panel lookup
on (plant_id, year_maj). Station-to-cell mapping is the identical nearest-cell
containment used by HYSPLITWeights, so the daily measure is directly comparable
to the annual one.

Outputs (keyed to merge onto smokeso2_{daily,monthly,annual}.csv):
    station_hysplit_daily.csv    station_id, date,     e_{coal,oil,gt}_k1..k7
    station_hysplit_monthly.csv  station_id, ym,       ...
    station_hysplit_annual.csv   station_id, year_maj, ...

chris.hockey@rhul.ac.uk
"""
...
sys.path.insert(0, str(
    ...
    "code" / "simulation"))  # noqa: E402

from hysplit.paths import ENSEMBLE_RUNS_DIR

# ==============================================================================
# Config -- wire these to your tree
# ==============================================================================
# cdump_<plant>_<year>_k<n>/
PANEL_PATH = Path("data/final/cegb_panel_with_stacks.csv")
STATIONS_PATH = Path(
    "data/intermediate/pollution_stations/smokeso2_annual.csv")
OUT_DIR = Path("data/final/pollution_stations")

YEAR_MAJ = 1981          # calibration financial year
# daily sampling window (for the end-stamp midpoint fix)
SAMPLE_HRS = 24
FUELS = ("coal", "oil", "gt")
