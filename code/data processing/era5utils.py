...
def getvars(means=False, tm=1, levtype='pl', instant=True):
    ...
    sname['TEMP'] = ['t', '130', '1.0', '130.128',
                     'temperature', '0', '0']  # units K
    sname['UWND'] = ["u", '131', '1.0', '131.128',
                     'u_component_of_wind', '2', '2']  # units m/s
    sname['VWND'] = ["v", '132', '1.0', '132.128',
                     'v_component_of_wind', '2', '3']  # units m/s
    sname['WWND'] = ["w", '135', '0.01', '135.128', 'vertical_velocity',
                     '2', '8']  # units Pa/s. convert to hPa/s for HYSPLIT
    sname['RELH'] = ["r", '157', '1.0',
                     '157.128', 'relative_humidity']  # units %
    # units m^2 / s^2. Divide by 9.8m/s^2 to get meters.
    sname['HGTS'] = ["z", '129', '0.102', '129.128', 'geopotential', '3', '4']
    ...
    sname['SPHU'] = ['q', '133', '1.0', '133.128',
                     'specific_humidity', '1', '0']  # units kg/kg
    # eta-coordinate vertical velocity units s^-1
    sname['ZWND'] = ['etadot', '77', '1.0', '133.128', '-1', '-1']
    sname['LNSP'] = ['lnsp', '152', '1.0',
                     '152.128', '', '3', '25']  # log pressure

    # 2D/surface analyses fields.
    # REQUIRED
    # units K         #Analysis (needed) ERA5
    sname['T02M'] = ['2t', '167', '1.0', '167.128', '2m_temperature']
    # units m/s       #Analysis (needed) ERA5
    sname['U10M'] = ['10u', '165', '1.0', '165.128', '10m_u_component_of_wind']
    # units m/s       #Analysis (needed) ERA5
    sname['V10M'] = ['10v', '166', '1.0', '166.128', '10m_v_component_of_wind']
    # OPTIONAL
    # Instantaneous. Units Pa        #multiplier of 0.01 for hPa.
    sname['PRSS'] = ['sp', '134', '0.01', '134.128', 'surface_pressure']
    # Instantaneous. Units Pa        #multiplier of 0.01 for hPa.
    sname['MSLP'] = ['msl', '151', '0.01', 'xxx.xxx',
                     'mean_sea_level_pressure', '3', '0']
    sname['TCLD'] = ['tcc', '164', '1.0', '164.128',
    ...
    sname['SHGT'] = ["z", '129', '0.102', '129.128',
    ...
    sname['PBLH'] = ["blh", '159', '1.0', '159.128',
                     'boundary_layer_height']  # boundary layer height : units m
    ...
    sname['TPP1'] = ['tp', '228', '1.0', '228.128', 'total_precipitation']
    # TPP3 is for the ensemble output which is every 3 hours.
    # Accumulated precipitation. units of m. multiplier is 1.
    sname['TPP3'] = ['tp', '228', '1.0', '228.128', 'total_precipitation']
    # forecast surface roughnes : units m
    sname['RGHS'] = ['fsr', '244', '1.0',
                     "244.128", 'forecast_surface_roughness']

    # It looks like the means are not output every hour so do not use them.
    if means:
        # units W/m^2 (surface sensible heat flux)
        sname['SHTF'] = ['msshf', '146', '1.0', '33.235']
        # latent heat flux. same as sshf
        sname['LTHF'] = ['mslhf', '34', '1.0', '34.235']
    elif instant:
        ...
        sname['LTHF'] = ['slhf', '147', amult, '147.128',
                         'surface_latent_heat_flux']  # same as sshf
    else:
        ...
        sname['SHTF'] = ['sshf', '146', amult,
                         '146.128', 'surface_sensible_heat_flux']
    # HYSPLIT convention is that upward sensible heat flux should be positive.
    # Multiply by -1
        sname['LTHF'] = ['slhf', '147', amult, '147.128',
                         'surface_latent_heat_flux']  # same as sshf
    ...
    sname['USTR'] = ['zust', '3', '1.0', '3.228', 'friction_velocity']
...
def pressure_levels(toplevel=1):
    levs = list(range(750, 1025, 25)) + list(range(300, 750, 50)) + \
        list(range(100, 275, 25)) + [1, 2, 3, 5, 7, 10, 20, 30, 50, 70]
...
# Added by Chris:
# If you need to check the GRIB file name (in shell):
# grib_ls -p shortName,paramId era5_sfc_an_1980_01.grib | sort -u | head -30
