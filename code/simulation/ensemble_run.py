...
Crosses every plant operating in the calibration year with the ensemble
members, running one HYSPLIT simulation per (plant, member). Sensible heat is
read straight from the panel's precomputed `heat_w_k1..k7` columns, so this
depends only on the panel, not on any PlantYear heat field. Coal, oil and gas
turbines all vary with the member, so every plant runs once per member
(105 plants x 7 = 735 runs for 1981/82).

Per-job CSV rows are appended to RUN_LOG_PATH:
    timestamp, batch_id, hostname, plant_id, year_maj, member_tag, heat_w,
    status, duration_seconds, kernel_path, error_type, error_msg

Status is OK / SKIPPED (kernel existed) / FAIL. Failures do not abort the
batch. Idempotent via AnnualKernel.already_done().
"""
...
from hysplit.paths import PANEL_PATH, ENSEMBLE_ROOT, DROPBOX_DIR
...
MAX_WORKERS = 5
N_JOBS = 5            # dress rehearsal: first N jobs. None for full run.
...
def _heat_cols(df: pd.DataFrame) -> list[str]:
    """Panel heat columns, ordered heat_w_k1, heat_w_k2, ..."""
    return sorted(
...
def _run_one(plant_id: str, year_maj: int, k_num: int, heat_w: float) -> dict:
    """Run one (plant, member) kernel. Returns a dict of log fields."""
    t0 = time.time()
    tag = f"k{k_num}"
    base = {
