...
(lat, lon) annual kernel by aggregating the daily-mean concentration
...
class AnnualKernel:
    ...
    Runs the continuous-release FY simulation (idempotent) and aggregates
    the daily concentration records to produce a (lat, lon) kernel. The
    result is saved to NetCDF with fuel input stored as an attribute for
    downstream scaling.
    """
    ...
    def __post_init__(self) -> None:
        self.run = HYSPLITRun(plant_year=self.plant_year)
        ...
        result = xr.Dataset(
            ...
            attrs={
                ...
                "description": (
                    ...
                    "a short tail for late-emitted particles to clear the "
                    "domain. Units are concentration per unit emission; "
                    "multiply by fuel input (stored here) for plant-"
                    "specific exposure."
                ),
            },
        )
